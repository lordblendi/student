module.exports = function () {
    ...
    this.Then(/^I should see the demonstrator's name and the entry test grade: (\d+) \("([^"]*)"\)$/, function (grade, name, callback) {
        var text = grade + " (" + name + ")";
        this.world.text('#afterentrytest', text, callback);
    });

    this.Then(/^I should see the report grade and the evaluator's name: (\d+) \("([^"]*)"\)$/, function (grade, name, callback) {
        var text = grade + " (" + name + ")";
        this.world.text('#afterreportgrade', text, callback);
    });

    this.Then(/^I should see the laboratory grade and the demonstrator's name: (\d+) \("([^"]*)"\)$/, function (grade, name, callback) {
        var text = grade + " (" + name + ")";
        this.world.text('#afterlaboratorygrade', text, callback);
    });

    this.Then(/^I should see my laboratory review$/, function (callback) {
    ...
    this.Then(/^I should see my report review$/, function (callback) {
